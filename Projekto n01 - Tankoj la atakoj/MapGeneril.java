...
public class MapGeneril extends JFrame implements KeyListener, ChangeListener {
    ...
    Random phase;
    private static int mapsize = 1024, ioo = 0;
//  private static float[] amplitudes = {0.2f, 0.5f, 1.0f, 0.7f, 0.5f, 0.4f};
    private static float[] amplitudes = {1.0f, 0.75f, 0.5f, 0.25f, 0.125f, 0.062f};
//  private static int[] frequencies = {1, 2, 4, 8, 16, 32};
    ...
    public void printt(boolean update) {
        if (!update) {
            return;
        }
        ...
        ger.setColor(Color.WHITE);
        ger.fillRect(0, 0, mapsize, offsetMap + 155);
        ger.setColor(Color.BLACK);
        for(int x = 0; x < mapsize; x++) {
            xx = x + xLudant;
            if(xx<0) 
